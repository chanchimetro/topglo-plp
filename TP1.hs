...
recCircuito :: (Caja -> a) -> (Circuito -> Circuito -> a -> a -> a) -> (Caja -> Circuito -> Circuito -> Caja -> a -> a -> a) -> Circuito -> a
recCircuito fCaja fSerie fParalelo circuito = case circuito of
  ...
  (Paralelo caja1 circ1 circ2 caja2) -> fParalelo caja1 circ1 circ2 caja2 (rec circ1) (rec circ2)
  where
...
foldCircuito fCaja fSerie fParalelo =
  recCircuito
    ...
    (\caja1 _ _ caja2 recCirc1 recCirc2 -> fParalelo caja1 recCirc1 recCirc2 caja2)
...
invertido =
  foldCircuito
    Caja
    (flip Serie)
    (\caja1 circ1 circ2 caja2 -> Paralelo caja2 circ2 circ1 caja1)
...
hayCaminoIluminado =
  foldCircuito
    (==on)
    (&&)
    (\caja1 circ1 circ2 caja2 -> (caja1==on) && (circ1 || circ2) && (caja2==on))
...
cantidadPrendidas =
  foldCircuito
    ...
    (+)
    (\caja1 circ1 circ2 caja2 -> estaPrendida caja1 + circ1 + circ2 + estaPrendida caja2)
    where
      estaPrendida caja = if caja==on then 1 else 0
...
esCircuitoProlijo =
  recCircuito
    ...
    (\_ _ _ _ recCirc1 recCirc2 -> recCirc1&&recCirc2)

-- 8: circuitoEmprolijado
--No funciona ya que c2 puede ser una Serie!
...
resistenciaCircuito =
  foldCircuito
    (const 1.0)
    (+)
    (\_ recCirc1 recCirc2 _ -> 1.0 + ((recCirc1 * recCirc2) / (recCirc1 + recCirc2)) + 1.0)

subCircuitoMásResistente :: Circuito -> Circuito
subCircuitoMásResistente =
  recCircuito
    Caja
    ( \circ1 circ2 recCirc1 recCirc2 -> masResistente (masResistente recCirc1 recCirc2) (Serie circ1 circ2))
    ( \caja1 circ1 circ2 caja2 recCirc1 recCirc2 -> masResistente (masResistente recCirc1 recCirc2) (Paralelo caja1 circ1 circ2 caja2))
    where
      masResistente circ1 circ2 = if resistenciaCircuito circ1 > resistenciaCircuito circ2 then circ1 else circ2
...
Por Principio de Inducción Estructural sobre Circuitos, basta probar los siguientes casos:
...
- Prueba de LEMA: Debo probar que
  ...
  Por Lema de Generación de Cajas, si caja:: Caja entonces
    1) O bien caja = Nada
    2) O bien (∃ b:: Bool) caja = Bombilla b
  ...
  Luego, para probar LEMA basta con demostrar cada caso:
    ...
    2) Caso (∃ b:: Bool) caja = Bombilla b: Por Lema de Generación de Booleanos, si b:: Bool entonces
      I) O bien b = True
      II) O bien b = False

      I) Pruebo Caso caja = Bombilla True:
      ...
      II) Pruebo Caso caja = Bombilla False:
